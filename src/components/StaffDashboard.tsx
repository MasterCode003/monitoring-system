...
import { Search, User, PieChartIcon, LogOut, ChevronDown, ChevronUp } from 'lucide-react';
...
const StaffDashboard: React.FC = () => {
  ...
  const stats = [
    ...
    { name: 'Upload', value: 30, color: '#8B5CF6' },
    { name: 'Reviewers Info', value: 50, color: '#F59E0B' },
  ];
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'upload':
      case 'reviewers-info':
        return <div className="text-center text-gray-500 mt-8">No content available for {activeTab}</div>;
      default:
        return (
      ...
      <div className="w-64 bg-blue-600 text-white p-6">
        ...
        <nav>
          <ul className="space-y-2">
            ...
            <li>
              <button
                onClick={() => setPreReviewOpen(!preReviewOpen)}
                className={`flex items-center justify-between w-full text-left py-2 px-4 rounded hover:bg-blue-700 transition duration-200 ${activeTab.startsWith('pre-review') ? 'bg-blue-700' : ''}`}
              >
                <span>Pre-Review</span>
                {preReviewOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
              </button>
              {preReviewOpen && (
                <ul className="ml-4 mt-2 space-y-2">
                  <li>
                    <button
                      onClick={() => setActiveTab('pre-review-upload')}
                      className={`block w-full text-left py-1 px-4 rounded hover:bg-blue-700 transition duration-200 ${activeTab === 'pre-review-upload' ? 'bg-blue-700' : ''}`}
                    >
                      Upload
                    </button>
                  </li>
                  <li>
                    <button
                      onClick={() => setActiveTab('pre-review-records')}
                      className={`block w-full text-left py-1 px-4 rounded hover:bg-blue-700 transition duration-200 ${activeTab === 'pre-review-records' ? 'bg-blue-700' : ''}`}
                    >
                      Records
                    </button>
                  </li>
            ...
            <li>
              <button
                ...
                className={`block w-full text-left py-2 px-4 rounded hover:bg-blue-700 transition duration-200 ${activeTab === 'upload' ? 'bg-blue-700' : ''}`}
              >
                Upload
              </button>
            </li>
            <li>
              <button
                onClick={() => setActiveTab('reviewers-info')}
                className={`block w-full text-left py-2 px-4 rounded hover:bg-blue-700 transition duration-200 ${activeTab === 'reviewers-info' ? 'bg-blue-700' : ''}`}
              >
                Reviewers Info
              </button>
            </li>
